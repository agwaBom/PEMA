...
#from knn_word_prediction import KNN_Dstore
...
class GenerationMixin:
    ...
    # this is PEMA interpolation which the PEFT method based on LoRA
    def lora_interpolation(self, outputs, next_tokens_scores):
        # if there are no more labels, then we are done
        if self.label_ids.shape[-1] != 0:
            
            # Fetch query (hidden state of last token)
            if outputs.hidden_states[0].shape[1] != 1:
                query = outputs.hidden_states[0][:, -1].unsqueeze(1)
            else:
                query = outputs.hidden_states[0]

            # this acts as pop for the label
            label = self.label_ids[:, 0]
            self.label_ids = self.label_ids[:, 1:]

            if self.beam_search_bool:
                beam_size = query.shape[0]//self.label_ids.shape[0]
                # repeat the label for each beam
                label = torch.repeat_interleave(label, beam_size, dim=0)

            if self.lora_trained:
                self.lora_linear.eval()
                with torch.no_grad():
                    lora_query, query = self.lora_linear(query.to(torch.float32))
                    lm_output = self.lm_head_1(self.final_layer_norm(lora_query))
                    lm_output = lm_output.log_softmax(dim=-1)
                    lm_output = lm_output.to(torch.float32)
                    query = query.half()

            next_tokens_scores = next_tokens_scores.unsqueeze(dim=1).log_softmax(dim=-1)
            logits_to_decode = next_tokens_scores.to(torch.float32)
            opt_logits = logits_to_decode.reshape((-1, logits_to_decode.shape[-1]))
            opt_logits = opt_logits.reshape((logits_to_decode.shape[0], logits_to_decode.shape[1], -1))
            
            coeff = self.lambda_apply

            combine_probs = torch.stack([opt_logits, lm_output], dim=0)
            # combine_probs [2, batch_size, seq_length]
            coeffs = torch.ones_like(combine_probs)
            # coeffs [2, batch_size, seq_length] : all 1s
            # [1, batch_size, seq_length]
            coeffs[0] = torch.log(1 - coeff).reshape((coeff.shape[0], 1, 1))
            coeffs[1] = torch.log(coeff).reshape((coeff.shape[0], 1, 1))
            curr_prob = torch.logsumexp(combine_probs + coeffs, dim=0)
        else:
            # if there are no more labels, then we are done
            curr_prob = next_tokens_scores
        return curr_prob
    ...
    def generate(
        ...
        **kwargs,
        ...
        self.interpolation_bool = opt_args.interpolation_bool
        self.lora_trained = opt_args.lora_trained

        # interpolation setting
        # if interpolation_bool is True, then we use interpolation between LM and PEMA
        if self.interpolation_bool:
            from train_rct_pd import LoraLinear
            self.lora_linear = LoraLinear(opt_args.decoder_embed_dim, opt_args.decoder_embed_dim, r=opt_args.num_rank, lora_alpha=1, lora_dropout=0.2).to('cuda')
            self.lora_linear.load_state_dict(torch.load(opt_args.lora_linear_path))

            # remove after test
            self.final_layer_norm = nn.LayerNorm(opt_args.decoder_embed_dim, elementwise_affine=True).to('cuda')
            self.lm_head_1 = nn.Linear(opt_args.decoder_embed_dim, 50272, bias=False).to('cuda')
            self.final_layer_norm.load_state_dict(torch.load(opt_args.lm_norm_path, map_location='cuda:0'))
            self.lm_head_1.load_state_dict(torch.load(opt_args.head_path, map_location='cuda:0'))
            self.lm_head_1.requires_grad_(False)
            ...
            self.tokenizer = GPT2Tokenizer.from_pretrained("facebook/opt-iml-max-1.3b", padding_side='left', _from_pipeline='text-generation')
    ...
    def greedy_search(
        ...
        **model_kwargs,
        ...
        if self.interpolation_bool == True:
            # If I can gradually decrease the value of lambda, I can get a better result.
            self.label_len = self.label_ids.ge(2).sum(dim=-1)

            lambda_max = self.opt_args.lmbda
            
            self.lambda_current = torch.full(self.label_len.shape, lambda_max).to(self.label_ids.device)
            self.lambda_apply = self.lambda_current
            
            self.lambda_step = lambda_max / self.label_len
        ...
        while True:
            ...
            self.beam_search_bool = False
            # interpolate with KNN
            if self.interpolation_bool == True:
                if self.opt_args.lora_decoder == True:
                    next_token_logits = self.lora_interpolation(outputs, next_token_logits)
                next_token_logits = next_token_logits.squeeze(dim=1)

                # Gradual unrolling
                # Only run when lambda_current is greater than 0             
                if (self.lambda_current-self.lambda_step > 0).all() and self.opt_args.gradual_unrolling == True:
                    self.lambda_current -= self.lambda_step
                    # exponential decay
                    self.lambda_apply = (self.lambda_current**2/self.opt_args.gradual_unrolling_min)+(self.opt_args.gradual_unrolling_min-1)/self.opt_args.gradual_unrolling_min
    ...
    def sample(
        ...
        **model_kwargs,
        ...
        while True:
            ...
            # sample
            # next_token_scores = self.knn_interpolation(outputs, next_token_scores)
            # from transformers import GPT2Tokenizer
            # tokenizer = GPT2Tokenizer.from_pretrained("facebook/opt-iml-max-30b", padding_side='left', _from_pipeline='text-generation')
            # tokenizer.decode(next_tokens)
            # interpolate with KNN
            if self.interpolation_bool == True:
                next_token_logits = self.knn_interpolation(outputs, next_token_logits)
                next_token_logits = next_token_logits.squeeze(dim=1)
